...
async def animator(media, mainevent, textevent):
    ...
    SaViorOp = await mainevent.client.download_media(media, Config.TEMP_DIR)
    await textevent.edit("__🎞Converting into Animated sticker..__")
    await runcmd(
        f"ffmpeg -ss 00:00:00 -to 00:00:02.900 -i {SaViorOp} -vf scale={w}:{h} -c:v libvpx-vp9 -crf 30 -b:v 560k -maxrate 560k -bufsize 256k -an animate.webm"
    )  # pain
    os.remove(SaViorOp)
...
async def waifutxt(text, chat_id, reply_to_id, bot):
    ...
    savior = await sticcers[0].click("me", hide_via=True)
    if savior:
        await bot.send_file(int(chat_id), savior, reply_to=reply_to_id)
        await savior.delete()
