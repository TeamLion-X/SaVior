...
class SaViorGloballist(BASE):
    __tablename__ = "saviorglobal_list"
    ...
    def __repr__(self):
        return "<SaVior global values '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, SaViorGloballist)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


SaViorGloballist.__table__.create(checkfirst=True)

SAVIORGLOBALLIST_INSERTION_LOCK = threading.RLock()
...
def add_to_list(keywoard, group_id):
    with SAVIORGLOBALLIST_INSERTION_LOCK:
        broadcast_group = SaViorGloballist(keywoard, str(group_id))
...
def rm_from_list(keywoard, group_id):
    with SAVIORGLOBALLIST_INSERTION_LOCK:
        broadcast_group = SESSION.query(SaViorGloballist).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_list(keywoard):
    with SAVIORGLOBALLIST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(SaViorGloballist.keywoard)
            .filter(SaViorGloballist.keywoard == keywoard)
            .delete()
        )
...
def get_list_keywords():
    try:
        chats = SESSION.query(SaViorGloballist.keywoard).distinct().all()
...
def num_list():
    try:
        return SESSION.query(SaViorGloballist).count()
    finally:
        SESSION.close()
...
def num_list_keywords():
    try:
        return SESSION.query(func.count(distinct(SaViorGloballist.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_lists():
    try:
        ...
        all_groups = SESSION.query(SaViorGloballist).all()
        for x in all_groups:
