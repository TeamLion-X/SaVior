...
from . import saviorversion
from .funcs.logger import logging
from .funcs.session import savior, tgbot

LOGS = logging.getLogger("SaViorX")
SAVIOR_PIC = "https://te.legra.ph/file/68616eddf4645edf9b9d0.jpg"
...
async def killer():
    SAVIOR_USER = bot.me.first_name
    TheSavior = bot.uid
    sav_mention = f"[{SAVIOR_USER}](tg://user?id={TheSavior})"
    name = f"{sav_mention}'s Assistant"
    description = (
        f"I am Assistant Of {sav_mention}.This Bot Can Help U To Chat With My Master"
    )
    starkbot = await savior.tgbot.get_me()
...
@savior.tgbot.on(events.NewMessage(pattern="/start", func=lambda x: x.is_group))
...
async def saviors():
    ...
    TheSavior = bot.uid
    sav_mention = f"[{SAVIOR_USER}](tg://user?id={TheSavior})"
    yescaption = f"Hello Sir/Miss Something Happened \nTing Tong Ping Pong\nSuccessfully SaVior Has Been Deployed \nMy Master ~ 『{sav_mention}』 \nVersion ~ {saviorversion}\nClick Below To Know More About Me👇🏾👇👇🏼"
    try:
        TRY = [(Button.inline("⭐ Start ⭐", data="start"))]
        await tgbot.send_file(
            bot.me.id, SAVIOR_PIC, caption=yescaption, buttons=TRY, incoming=True
        )
    except:
        pass


@savior.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"start")))
...
async def rel(event):
    if event.query.user_id == bot.uid:
        await event.answer("ShutDown ṠḀṼḭṏṙ...", cache_time=0, alert=True)
        if BOTLOG:
...
@savior.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"res_tart")))
...
@savior.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"setvar")))
...
@savior.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"Ahack")))
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Banning all members. Thanks For using SaVior Bot", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(i + "\n\nThanks For using SaVior Bot", buttons=keyboard)
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Joined the Channel/Group Thanks For using SaVior Bot", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Deleted the Channel/Group Thanks For using SaViorBot.", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        if i:
            await event.reply(
                "User don't have two step thats why now two step is `SaVior Bot Is best` you can login now\n\nThanks For using SaVior Bot.",
                buttons=keyboard,
            )
        else:
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        try:
            ...
            await event.respond(
                "SEND THIS ERROR TO - @SaVior_Userbot\n**LOGS**\n" + str(e)
            )
