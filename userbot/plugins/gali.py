...
from . import savior, saviormemes, useless
...
async def abujjnsing(abused):
    "random abuse string"
    reply_text = random.choice(saviormemes.ABUSE_STRINGS)
...
async def fuckloedd(abusehard):
    "random gali string"
    reply_text = random.choice(saviormemes.ABUSEHARD_STRING)
...
async def met(e):
    "random gali string"
    txt = random.choice(saviormemes.RENDISTR)
    await eor(e, txt)
...
async def chuya(fuks):
    "random gali string"
    reply_text = random.choice(saviormemes.CHU_STRINGS)
...
async def thaos(thanos):
    "random gali string"
    reply_text = random.choice(saviormemes.THANOS_STRINGS)
...
async def _(event):
    "fun animation"
    saviorevent = await eor(event, "`kiss`")
    animation_interval = 0.2
    animation_ttl = range(100)
    animation_chars = ["🤵       👰", "🤵     👰", "🤵  👰", "🤵💋👰"]
    for i in animation_ttl:
        await asyncio.sleep(animation_interval)
        await saviorevent.edit(animation_chars[i % 4])
...
async def _(event):
    "fun animation"
    saviorevent = await eor(event, "`fuking....`")
    animation_interval = 0.2
    animation_ttl = range(100)
...
async def _(event):
    "fun animation"
    saviorevent = await eor(event, "`sex`")
    animation_interval = 0.2
    animation_ttl = range(100)
    animation_chars = ["🤵       👰", "🤵     👰", "🤵  👰", "🤵👼👰"]
    for i in animation_ttl:
