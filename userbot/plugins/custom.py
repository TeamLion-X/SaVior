    ...
    if vname in vlist:
        ...
        if cmd == "set":
            if vname == "DEFAULT_USER":
                ...
                USERINFO = await savior.get_entity(legebd.uid)
                FULL_USERINFO = (await savior(GetFullUserRequest(savior.uid))).full_user
                ...
                if FULL_USERINFO.about:
                    ...
                    delgvar("DEFAULT_BIO")
                try:
                    photos = await savior.get_profile_photos(savior.uid)
                    myphoto = await savior.download_media(photos[0])
                ...
                if not vinfo and vname == "ALIVE_TEMPLATE":
                    return await eod(event, "Check @SaVior_Alive")
                if not vinfo:
                    return await eod(
                        event,
...
@savior.savior_cmd(
    ...
    info={
        "header": "To customize your SaViorX.",
        "options": {
...
async def custom_SaViorX(event):
    "To customize your SaViorX."
    reply = await event.get_reply_message()
    text = None
    if reply:
        text = reply.text
...
@savior.savior_cmd(
    ...
    info={
        "header": "To delete costomization of your SaVior.",
        "options": {
...
async def custom_ksks(event):
    "To delete costomization of your SaVior."
    input_str = event.pattern_match.group(1)
    if input_str == "pmpermit":
