...
from ..helpers.utils import _format, _saviorutils, reply_id
...
async def currency(event):
    ...
    try:
        ...
        symbols = await AioHttp().get_raw(
            "https://raw.githubusercontent.com/TheSaVior/RESOURCES/master/Resources/Data/currency.py"
        )
...
async def _(event):
    ...
    chat = "@VS_Robot"
    saviorevent = await eor(event, " `Sliding my tip, of fingers over it`")
    async with event.client.conversation(chat) as conv:
        ...
        if not input_str:
            return await eor(saviorevent, response4.text)
...
async def parseqr(event):
    ...
    saviorevent = await eor(event, "`Decoding....`")
    ...
    t_response, e_response = (await _saviorutils.runcmd(command_to_exec))[:2]
    if os.path.exists(downloaded_file_name):
        os.remove(downloaded_file_name)
    soup = BeautifulSoup(t_response, "html.parser")
    try:
        qr_contents = soup.find_all("pre")[0].text
        await eor(saviorevent, f"**The decoded message is :**\n`{qr_contents}`")
    except IndexError:
        result = soup.text
        await eor(saviorevent, f"**Failed to Decode:**\n`{result}`")
...
async def _(event):
    ...
    try:
        ...
        return await saviorevent.edit(str(e))
    end = datetime.now()
    ms = (end - start).seconds
    await eod(saviorevent, "Created BarCode in {} seconds".format(ms))
...
async def _(event):
    ...
    saviorevent = await eor(event, "`processiong...........`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
    ...
    if not r.ok:
        return await saviorevent.edit("xkcd n.{} not found!".format(xkcd_id))
    data = r.json()
    year = data.get("year")
...
Year: {}""".format(
    ...
    await saviorevent.edit(output_str, link_preview=True)
