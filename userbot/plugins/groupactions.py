...
async def ban_user(chat_id, i, rights):
    try:
        await savior(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
...
async def _(event):
    ...
    saviorevent = await eor(event, "__Unbanning all banned accounts in this group.__")
    succ = 0
    total = 0
    type = False
...
async def rm_deletedacc(show):  # sourcery no-metrics
    ...
    if con != "clean":
        ...
        if type != " -r":
            ...
            savioradmin = await is_admin(show.client, show.chat_id, show.client.uid)
            if not savioradmin:
                return await eod(
                    event,
