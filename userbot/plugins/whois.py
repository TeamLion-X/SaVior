...
async def _(event):
    ...
    if not replied_user:
        return
    saviorevent = await eor(event, "`Fetching userinfo wait....`")
...
async def who(event):
    ...
    if not replied_user:
        return
    savior = await eor(event, "`Fetching userinfo wait....`")
    replied_user = await event.client(GetFullUserRequest(replied_user.id))
    try:
    ...
    try:
        ...
        await savior.edit(caption, parse_mode="html")
