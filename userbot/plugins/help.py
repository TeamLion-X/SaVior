...
async def grpinfo():
    outstr = "**Plugins in SaVior are:**\n\n"
    outstr += f"**✨Usage : ** `{cmdprefix}help <plugin name>`\n\n"
    category = ["admin", "bot", "fun", "misc", "tools", "utils", "extra", "useless"]
    for savior in category:
        plugins = GRP_INFO[savior]
        outstr += f"**{hemojis[savior]} {savior.title()} **({len(plugins)})\n"
...
async def cmdlist():
    outstr = "**Total list of Commands in your SaVior are :**\n\n"
...
@savior.savior_cmd(
    ...
    info={
        "header": "To get guide for SaVior.",
...
async def _(event):
    ...
    if not input_str:
        ...
        for cmd in cmds:
            outstr += f"  - `{cmdprefix}{cmd}`\n"
        outstr += f"**✨ Usage : ** `{cmdprefix}help -l <command name>`"
    await eor(event, outstr, aslink=True, linktext="Total Commands of SaVior are :")
