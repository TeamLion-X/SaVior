...
async def echo(event):
    "To start chatbot to the user messages by SaVior"
    if event.reply_to_msg_id is None:
        return await eor(event, "`Reply to a User's message to echo his messages`")
    saviorevent = await eor(event, "`See Magic`")
...
async def sareply(event):
    if sis_echo(event.chat_id, event.sender_id) and (
        ...
        loll = event.text
        redeye = loll.replace(" ", "%20")
        url = f"https://kukiapi.xyz/api/apikey=1356469075-KUKIkq4WMg5FV4/SaViorAssistance/SaVior/message={redeye}"
