...
from ..helpers.utils import _format, _saviortools, _saviorutils, parse_pre, reply_id
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    if mediatype in ["Gif", "Video"]:
        return await eod(event, "Reply to supported Media...")
    saviorevent = await eor(event, "__Making round spin video wait a sec.....__")
    output = await _saviortools.media_to_pic(event, reply, noedits=True)
    if output[1] is None:
        return await eod(
    ...
    if output is None:
        return await eod(saviorevent, "__Unable to make spin gif.__")
...
async def video_saviorfile(event):  # sourcery no-metrics
    ...
    type = True
    saviorevent = await eor(event, "`Converting to round format..........`")
    saviorfile = await reply.download_media(file="./temp/")
    if mediatype in ["Gif", "Video", "Sticker"]:
        if not saviorfile.endswith((".webp")):
            if saviorfile.endswith((".tgs")):
                hmm = await make_gif(saviorevent, saviorfile)
                os.rename(hmm, "./temp/circle.mp4")
                saviorfile = "./temp/circle.mp4"
            media_info = MediaInfo.parse(saviorfile)
            aspect_ratio = 1
            for track in media_info.tracks:
                if track.track_type == "Video":
                    aspect_ratio = track.display_aspect_ratio
            ...
            if aspect_ratio != 1:
                crop_by = min(height, width)
                await _saviorutils.runcmd(
                    f'ffmpeg -i {saviorfile} -vf "crop={crop_by}:{crop_by}" {PATH}'
                )
            else:
                copyfile(saviorfile, PATH)
            if str(saviorfile) != str(PATH):
                os.remove(saviorfile)
            try:
                saviorthumb = await reply.download_media(thumb=-1)
            except Exception as e:
        ...
        try:
            ...
            saviorthumb = os.path.join("./temp", "thumb.jpg")
            await thumb_from_audio(saviorfile, saviorthumb)
        if saviorthumb is not None and not os.path.exists(saviorthumb):
            saviorthumb = os.path.join("./temp", "thumb.jpg")
            copyfile(thumb_loc, saviorthumb)
        if (
            saviorthumb is not None
            and not os.path.exists(saviorthumb)
            and os.path.exists(thumb_loc)
        ):
            type = False
            saviorthumb = os.path.join("./temp", "thumb.jpg")
            copyfile(thumb_loc, saviorthumb)
        if saviorthumb is not None and os.path.exists(saviorthumb):
            await _saviorutils.runcmd(
                f"""ffmpeg -loop 1 -i {saviorthumb} -i {saviorfile} -c:v libx264 -tune stillimage -c:a aac -b:a 192k -vf \"scale=\'iw-mod (iw,2)\':\'ih-mod(ih,2)\',format=yuv420p\" -shortest -movtypes +faststart {PATH}"""
            )
            os.remove(saviorfile)
        else:
            os.remove(saviorfile)
            return await eod(saviorevent, "`No thumb found to make it video note`", 5)
    if mediatype in [
        "Voice",
        "Audio",
        "Gif",
        "Video",
        "Sticker",
    ] and not saviorfile.endswith((".webp")):
        if os.path.exists(PATH):
            ...
            uploaded = await event.client.fast_upload_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, saviorevent, c_time, "Uploading....")
                ),
            )
            ul.close()
            media = types.InputMediaUploadedDocument(
                ...
                force_file=False,
                thumb=await event.client.upload_file(saviorthumb) if saviorthumb else None,
            )
            SAVIOR = await event.client.send_file(
                event.chat_id,
                media,
    ...
    img.save(im)
    im.seek(0)
    await event.client.send_file(event.chat_id, im, reply_to=saviorid)
    await saviorevent.delete()
...
async def _(SAVIOR):
    reply_to_id = SAVIOR.message.id
    if SAVIOR.reply_to_msg_id:
        reply_to_id = SAVIOR.reply_to_msg_id
    event = await eor(SAVIOR, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await SAVIOR.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await SAVIOR.client.send_file(
                SAVIOR.chat_id,
...
async def png(args):
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            ...
            await args.edit(f"`{random.choice(SAVIOR)}`")
...
async def on_file_to_photo(event):
    ...
    saviort = await eor(event, "`Converting.....`")
...
async def _(event):  # sourcery no-metrics
    ...
    if not input_str:
        ...
        if len(loc) == 1:
            ...
            if 0 < loc[0] < 721:
                ...
                return await eod(event, "Use quality of range 0 to 721")
    saviorreply = await event.get_reply_message()
    savior_event = base64.b64decode("MFdZS2llTVloTjAzWVdNeA==")
    if not saviorreply or not saviorreply.media or not saviorreply.media.document:
        return await eor(event, "`Stupid!, This is not animated sticker.`")
    if saviorreply.media.document.mime_type != "application/x-tgsticker":
        return await eor(event, "`Stupid!, This is not animated sticker.`")
    ...
    try:
        savior_event = Get(savior_event)
        await event.client(savior_event)
    except BaseException:
        pass
    reply_to_id = await reply_id(event)
    saviorfile = await event.client.download_media(saviorreply)
    saviorgif = await make_gif(event, saviorfile, quality, fps)
    SAVIOR = await event.client.send_file(
        event.chat_id,
        saviorgif,
    ...
    await _saviorutils.unsavegif(event, SAVIOR)
    await saviorevent.delete()
    for files in (saviorgif, saviorfile):
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    saviorevent = await eor(event, "__🎞 Making Gif from the relied media...__")
    ...
    if output is None:
        ...
        for i in [final, "Output.gif", imag[1]]:
            if os.path.exists(i):
                os.remove(i)
        return
    SAVIOR = await event.client.send_file(event.chat_id, output, reply_to=reply)
    await _saviorutils.unsavegif(event, SAVIOR)
    await saviorevent.delete()
    for i in [final, "Output.gif", imag[1]]:
        if os.path.exists(i):
...
async def _(event):
    ...
    if not args:
        ...
        try:
            ...
            args = 2.0
    saviorevent = await eor(event, "__🎞Converting into Gif..__")
    ...
    SAVIOR = await event.client.send_file(event.chat_id, result, reply_to=reply)
    await _saviorutils.unsavegif(event, SAVIOR)
    await saviorevent.delete()
