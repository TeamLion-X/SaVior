...
@savior.savior_cmd(
    ...
    info={
        ...
        "description": "Reply to any external plugin(supported by SaVior) to install it in your bot.",
        "usage": "{tr}install",
    },
)
async def install(event):
    ...
    b = 1
    owo = event.text[9:]
    savior = await eor(event, "__Installing.__")
    if event.reply_to_msg_id:
        try:
            ...
            try:
                ...
                await eod(savior, f"{e}")
                return os.remove(downloaded_file_name)
        except Exception as e:
            await eod(savior, f"**Failed to Install** \n`Error`\n{str(e)}")
            return os.remove(downloaded_file_name)
